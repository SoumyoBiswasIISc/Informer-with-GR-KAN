...
parser = argparse.ArgumentParser(description='[Informer/GR-KAN Informer] Long Sequences Forecasting')
...
parser.add_argument('--do_predict', action='store_true', help='whether to predict unseen future data')
parser.add_argument('--mix', action='store_false', default=True, help='use mix attention in decoder')
parser.add_argument('--cols', type=str, nargs='+', help='certain cols from the data files as the input features')
parser.add_argument('--num_workers', type=int, default=0, help='data loader num workers')
parser.add_argument('--itr', type=int, default=2, help='experiments times')
parser.add_argument('--train_epochs', type=int, default=6, help='train epochs')
...
data_parser = {
    ...
    'ECL':   {'data': 'ECL.csv', 'T': 'MT_320', 'M': [321, 321, 321], 'S': [1, 1, 1], 'MS': [321, 321, 1]},
    'Solar': {'data': 'solar_AL.csv', 'T': 'POWER_136', 'M': [137, 137, 137], 'S': [1, 1, 1], 'MS': [137, 137, 1]},
    # === Agriculture-specific dataset entry ===
    'custom': {'data': 'integrated_dataset.csv', 'T': 'Ground_Truth', 'M': [8, 8, 1], 'S': [1, 1, 1], 'MS': [8, 8, 1]},
}
